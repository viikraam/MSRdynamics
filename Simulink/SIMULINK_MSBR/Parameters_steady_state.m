...
%% Nuclear Model (initial values from steady-state simulation)
n_frac0 = 1.027988731588695e+00; % initial fractional nuetron density n/n0 (n/cm^3/s)
P       = 5.715617347633142e+02; %556.0; % Thermal Power in MW
...
C0(1) = 2.081938258519982e+01; % ((beta(1))/Lam)*(1.0/(lam(1) - (exp(-lam(1)*tau_l) - 1.0)/tau_c));
C0(2) = 2.936813574414831e+01; % ((beta(2))/Lam)*(1.0/(lam(2) - (exp(-lam(2)*tau_l) - 1.0)/tau_c));
C0(3) = 7.165199547168344e+00; % ((beta(3))/Lam)*(1.0/(lam(3) - (exp(-lam(3)*tau_l) - 1.0)/tau_c));
C0(4) = 4.542053435975586e+00; % ((beta(4))/Lam)*(1.0/(lam(4) - (exp(-lam(4)*tau_l) - 1.0)/tau_c));
C0(5) = 3.713468726659037e-01; % ((beta(5))/Lam)*(1.0/(lam(5) - (exp(-lam(5)*tau_l) - 1.0)/tau_c));
C0(6) = 1.132819257835807e-01; % ((beta(6))/Lam)*(1.0/(lam(6) - (exp(-lam(6)*tau_l) - 1.0)/tau_c));
...
T0_f1b1  = 6.944408882425997e+02; % T0_f4;
T0_f1b2  = 6.745208085786668e+02; % T0_f4-(T0_f4-Tf_in)*(1/8); % in °C
T0_f1a1  = 6.546007289147340e+02; % T0_f4-(T0_f4-Tf_in)*(2/8); % in °C
T0_f1a2  = 6.346522578801772e+02; % T0_f4-(T0_f4-Tf_in)*(3/8); % in °C
T0_f2a1  = 6.147037868456204e+02; % T0_f4-(T0_f4-Tf_in)*(4/8); % in °C
T0_f2a2  = 5.930399821345674e+02; % T0_f4-(T0_f4-Tf_in)*(5/8); % in °C
T0_f2b1  = 5.713761774235144e+02; % T0_f4-(T0_f4-Tf_in)*(6/8); % in °C 
T0_f2b2  = 5.496905362117695e+02; % T0_f4-(T0_f4-Tf_in)*(7/8); % in °C


T0_Ba2   = 7.563894577306696e+02; % T0_b2; % in °C
T0_Ba1   = 7.268483924625359e+02; % T0_b2-(T0_b2-T_b_in)*(1/4); % in °C
T0_Bb2   = 6.973073271944021e+02; % T0_b2-(T0_b2-T_b_in)*(2/4); % in °C
T0_Bb1   = 6.676836875003301e+02; % T0_b2-(T0_b2-T_b_in)*(3/4); % in °C


T0_g1b   = 6.744688529079845e+02; % in °C
T0_g2b   = 5.504123767420686e+02; % in °C
T0_g1a   = 6.349384567895716e+02; % in °C
T0_g2a   = 5.935084724769273e+02; % in °C

T0_g3a   = 6.054973902139616e+02; % in °C
T0_g4a   = 7.345388909953654e+02; % in °C
T0_g3b   = 5.620995368424542e+02; % in °C
T0_g4b   = 6.754061178074082e+02; % in °C 
...
T0_BL    = 6.951553112471239e+02; % T0_Ba2;
T0_Bm    = 7.259413528842708e+02; 
...
T0_p1  = 6.488785041245277e+02; % Tp_in - 41.6675; % in °C
T0_p2  = 6.033161200064557e+02; % Tp_in - 2*41.6675; % in °C
T0_p3  = 5.656605075032402e+02; % Tp_in - 3*41.6675; % in °C
T0_p4  = 5.280048950000247e+02; %537.77; % in °C
T0_pm  = 6.033161200064557e+02; % T0_p2; % in °C

T0_t1  = 6.006295492584754e+02; % 596.87; % in °C
T0_t2  = 5.257845443970743e+02; % 520.71; % in °C

T0_s1  = 5.054017624727485e+02; % Ts_in + 36.25; % in °C
T0_s2  = 5.373320252427538e+02; % Ts_in + 2*36.25; % in °C
T0_s3  = 5.759668740901528e+02; % Ts_in + 3*36.25; % in °C
T0_s4  = 6.146017229375518e+02; % 599.4444; % in °C
...
% Initial conditions
T0_s5  = 6.204847061643086e+02; % 603.33;
T0_s6  = 6.263676893910654e+02; % 607.22;
T0_pb1 = 6.820007003452645e+02; % 648.88;
T0_pb2 = 6.380600478062580e+02; % 676.66; % fertile exit temp deg C
T0_t3  = 6.512427032547865e+02; % 633.6967; % tube temp
